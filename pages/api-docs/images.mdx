...
We have curated a set of [public models](/public-models) which anyone can use to generate following our API reference [here](https://reference.tryleap.ai/reference/inferencescontroller_create-1). 

You may also use custom models trained via fine-tuning. To get your custom models use `GET /v1/images/models` with your API key as described in the [Get Models](https://reference.tryleap.ai/reference/listallmodels)
endpoint documentation. You will need the model ID to query the generete image endpoint.

Here is a `python` example implementing image generation with our public Realistic Vision model. Get your [API Key](authentication) first
